...
['Online', 'Staged'].each { version ->
  ...
  // Group products by brand prefix and macro-category - ONLY those with valid picture!
  def byPrefix = [:].withDefault { [] }
  def byMacro = [:].withDefault { [] }
  
  products.each { p ->
    // STRICT IMAGE CHECK: only candidate targets that have a valid picture!
    if (p.picture != null && p.thumbnail != null) {
  ...
  products.each { p ->
    ...
    // SIMILAR: 4 candidates with images
    ...
    // CROSSELLING: 3 candidates from other macro-categories (with images)
...
println "SUCCESS: Recreated ${totalCreated} ProductReferences strictly requiring picture != null."
